...
Linear filters operate across spatial domains via discrete 2D spatial convolution. Given a continuous discrete image matrix $I$ and a localized bounding window known as a kernel $K$ of odd-integer dimensional extents $(2a+1)×(2a+1)$, the mathematically modified output coordinate $I$′$(x,y)$ is calculated via:
...
- $σ$ represents the standard deviation of the Gaussian distribution, controlling the width of the bell curve and governing the level of spatial smoothing.

Within this calculator's initialization pipeline, the kernel window size is fixed at an odd boundary constraint of $13×13$ pixels. By setting the explicit standard deviation parameter to zero $(σ=0)$, the underlying engine automatically calculates the optimal standard deviation using the following linear function of kernel width:

$$\sigma = 0.3 \cdot \left( \frac{\text{Kernel Size} - 1}{2} - 1 \right) + 0.8$$

For a hardcoded $13×13$ window footprint, this optimization resolves to:
...
For a continuous 2D intensity function $f(x,y)$, the Laplacian operator $(∇2)$ is defined as the sum of the unmixed second partial spatial derivatives:

$$\nabla^2 f = \frac{\partial^2 f}{\partial x^2} + \frac{\partial^2 f}{\partial y^2}$$

In the discrete pixel domain, this second-order derivative is approximated using finite central differences. For a standard $3×3$ spatial window, the finite difference approximation resolves to the following default structural matrix kernel:
...
The mean filter replaces the center pixel value with the unweighted arithmetic mean of all digital numbers enclosed within a localized window $W$ of dimensions $k×k$. The corresponding convolution kernel assigns an equal, uniform weight to all coefficient positions:

$$K_{\text{mean}}(i, j) = \frac{1}{k^2} \quad \forall \quad i, j \in W$$

Within this module's implementation, the spatial footprint is fixed at a $9×9$ pixel layout. This creates an unweighted box-filter kernel where each individual entry is defined as:
...
Unlike linear convolutional filters, the median filter does not use a weighted scalar dot product. Instead, it analyzes the neighborhood window $W$ of size $k×k$ centered at coordinates $(x,y)$, extracts all raw pixel values, sorts them in ascending numerical order, and assigns the exact middle value to the target pixel:
...
To evaluate these directional changes, the Sobel operator applies two separate $3×3$ convolutional kernels for horizontal ($G_x$​) and vertical ($G_y$​) derivative approximations:
...
- **Constructor Method (`__init__`) Input Arguments:**
    ...
    - `kernel_size` (`int`): Bounding odd integer dimension parameter (must be $3,5,$or $7$).
