...
Where $K$ is the number of land-cover classes. On the interactive path $N_{\text{train}} = K \times \text{\textit{sample number}}$. On the programmatic path $N_{\text{train}}$ is the length of `training_samples`.
...
3. Once the user records the total required number of clicks ($K \times \mathit{sample~number}$), the interaction window closes automatically, and the pipeline starts training the SVM model.
