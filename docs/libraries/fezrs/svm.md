...
Where $K$ is the number of land-cover classes. On the interactive path $N_{\text{train}} = K \times \text{sample&#95;number}$. On the programmatic path $N_{\text{train}}$ is the length of `training_samples`.
